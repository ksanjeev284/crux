/* CRUX block explorer — all consensus logic, ported from the Python reference.
   No dependencies. Everything below runs in the visitor's browser. */

const REPO = document.documentElement.dataset.repo || "ksanjeev284/crux";
const RAW = `https://raw.githubusercontent.com/${REPO}/main/`;

/* ---------- consensus constants (must match crux/consensus.py) ---------- */
...
const MAX_ADJUST = 4;
const POW_LIMIT_BITS = 0x2100ffff;
const GENESIS_BITS = 0x20040000;
const COINBASE_MATURITY = 10;
const INITIAL_SUBSIDY = 50n * COIN;
const PN = 44, PB = 42, SOL_BYTES = 8;
...
function headerCore(b) {
  return utf8(`${b.height}|${b.prev_hash}|${b.merkle_root}|${b.timestamp}|` +
              `${b.bits.toString(16).padStart(8, "0")}|${b.miner}|${b.algo ?? 1}|${b.nonce ?? 0}`);
}
const blockHash = b => hex(sha256d(cat(headerCore(b), utf8("|" + (b.solution ?? "")))));
const blockDigest = b => sha256d(cat(headerCore(b), utf8("|" + (b.solution ?? ""))));
...
/* ---------- subset-sum + hash-target proof of work ---------- */
function puzzleInstance(core) {
  const seed = sha256d(core);
...
function decodeSolution(blob) {
  const raw = unhex(blob || "");
  if (raw.length !== SOL_BYTES) throw new Error(`solution must be exactly ${SOL_BYTES} bytes`);
  return toBig(raw);
}
function checkPow(b) {
  let subset;
  try { subset = decodeSolution(b.solution); }
  catch (e) { return `malformed solution: ${e.message}`; }
  if (subset <= 0n || subset >= (1n << BigInt(PN))) return "subset out of range";
  const core = headerCore(b);
  const { nums, target } = puzzleInstance(core);
  let sum = 0n;
  for (let i = 0; i < PN; i++) if ((subset >> BigInt(i)) & 1n) sum += nums[i];
  if (sum !== target) return "subset does not solve the header's knapsack";
  const digest = toBig(blockDigest(b));
  const tgt = bitsToTarget(b.bits);
  if (digest > tgt) return `block hash is above the target (${fmtBits(b.bits)})`;
  return null;
}
...
function fmtAmount(grains) {
  const v = BigInt(grains);
